...
vec4 CalculateSpotLight(Light light, vec3 normal, vec3 fragPos, vec3 viewDir);
...
{
    //Ambient
    vec4 ambient = vec4(light.ambientColor, 1.0) * texture(material.diffuse[0], TexCoords);

    //Diffuse
    vec3 lightDir = normalize(light.direction);
    float diff = max(dot(normal, lightDir), 0.0);
    vec4 diffuse = vec4(light.diffuseColor * diff, 1.0) * texture(material.diffuse[0], TexCoords);

    //Specular
    vec3 reflectDir = reflect(-lightDir, normal);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
    vec4 specular = vec4(light.specularColor * spec, 1.0) * texture(material.specular[0], TexCoords);
